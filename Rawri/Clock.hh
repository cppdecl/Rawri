#ifndef __RAWRI__CLOCK__HH__
#define __RAWRI__CLOCK__HH__
...
namespace Rawri 
{
    ...
    namespace Clock
    {
        ...
        inline auto GetSecondsSinceEpoch() -> int
        {
            return std::chrono::duration_cast<std::chrono::seconds>(Now().time_since_epoch()).count();
        }

        inline auto TimePointToEpoch(TimePoint timePoint) -> int
        {
            return std::chrono::duration_cast<std::chrono::seconds>(timePoint.time_since_epoch()).count();
        }

        inline auto EpochToTimePoint(uint64_t secondsSinceEpoch) -> TimePoint
        {
        ...
        inline auto FormatGTTimer(uint32_t ms) -> std::string
        {
        ...
        inline auto FormatElapsed(TimePoint lastSomething, bool extended = true) -> std::string 
        {
            if (Clock::DiffSec(lastSomething) < 60)
                return std::format("{}", extended ? "a few secs ago" : std::format("{}s", (int32_t)Clock::DiffSec(lastSomething)));
            else if (Clock::DiffMin(lastSomething) < 60)
                return std::format("{}", extended ? "a few mins ago" : std::format("{}m", (int32_t)Clock::DiffMin(lastSomething)));
            else if (Clock::DiffHour(lastSomething) == 1)
                return std::format("{}", extended ? "an hour ago" : std::format("{}h", (int32_t)Clock::DiffHour(lastSomething)));
            else if (Clock::DiffHour(lastSomething) > 1 && Clock::DiffHour(lastSomething) < 24)
                return std::format("{}", extended ? std::format("{} hours ago", (int32_t)Clock::DiffHour(lastSomething)) : std::format("{}h", (int32_t)Clock::DiffHour(lastSomething)));
            else if (Clock::DiffDay(lastSomething) == 1)
                return std::format("{}", extended ? "a day ago" : std::format("{}d", Clock::DiffDay(lastSomething)));
            else 
                return std::format("{}", extended ? std::format("{} days ago", (int32_t)Clock::DiffDay(lastSomething)) : std::format("{}d", (int32_t)Clock::DiffDay(lastSomething)));
        }
        ...
        inline auto GetArcaneTimestamp() -> std::string 
        {
